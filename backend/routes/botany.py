...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  RESONANCE COMPATIBILITY — Predictive Synergy Engine
#  Calculates real-time element synergies in the garden
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# Reverse lookups
GENERATED_BY = {v: k for k, v in GENERATING.items()}  # Fire generated by Wood
CONTROLLED_BY = {v: k for k, v in CONTROLLING.items()}  # Earth controlled by Wood


@router.get("/botany/resonance/{element}")
async def get_resonance_compatibility(element: str, user=Depends(get_current_user)):
    """Calculate real-time synergies between selected element and garden plants."""
    ...
    for g in garden:
        plant_elem = g.get("element", "Earth")
        plant_name = g.get("plant_name", "Unknown")
        plant_mass = g.get("gravity_mass", 60)
        plant_freq = g.get("frequency", 639.0)
        stage_multiplier = {"Seed": 0.5, "Sprout": 0.7, "Sapling": 0.85, "Mature": 1.0, "Ancient": 1.2, "Transcendent": 1.5}.get(g.get("stage", "Seed"), 0.5)
        ...
        if plant_elem == element:
            synergy_type = "harmony"
            synergy_score = round(15 * stage_multiplier)
            description = f"Same element resonance amplifies {element} frequency"
        elif GENERATING.get(plant_elem) == element:
            synergy_type = "generating"
            synergy_score = round(20 * stage_multiplier)
            total_boost += synergy_score
            description = f"{plant_elem} generates {element} — creative flow"
        elif GENERATED_BY.get(element) == plant_elem:
            synergy_type = "generated_by"
            synergy_score = round(10 * stage_multiplier)
            total_boost += synergy_score
            description = f"{element} draws power from {plant_elem}"
        elif CONTROLLING.get(plant_elem) == element:
            synergy_type = "controlled"
            synergy_score = round(-15 * stage_multiplier)
            total_conflict += abs(synergy_score)
            description = f"{plant_elem} controls {element} — restraining force"
        elif CONTROLLED_BY.get(element) == plant_elem:
            synergy_type = "controlling"
            synergy_score = round(-10 * stage_multiplier)
            total_conflict += abs(synergy_score)
            description = f"{element} must overcome {plant_elem} resistance"
        else:
            synergy_score = round(5 * stage_multiplier)
            description = f"Indirect relationship via cycle distance"

        synergies.append({
            "garden_id": g.get("id"),
            "plant_name": plant_name,
            "plant_element": plant_elem,
            "plant_mass": plant_mass,
            "plant_frequency": plant_freq,
            "stage": g.get("stage", "Seed"),
    ...
    net_flow = total_boost - total_conflict
    forecast = "balanced"
    if net_flow > 20:
        forecast = "surge"
    elif net_flow > 5:
    ...
    # What would adding a plant of this element change?
    projected_score = net_flow + 15  # harmony bonus of a same-element plant
    projection_delta = projected_score - net_flow

    return {
        "element": element,
        "frequency": ELEMENT_FREQUENCY[element],
        ...
        "projection": {
            "if_add_same_element": projected_score,
            "delta": projection_delta,
            "recommendation": f"Adding a {element} plant would {'amplify' if projection_delta > 0 else 'further strain'} your garden's {element} resonance by {abs(projection_delta)} points",
        },
        "generates": GENERATING[element],
        "controls": CONTROLLING[element],
        "generated_by": GENERATED_BY.get(element),
        "controlled_by": CONTROLLED_BY.get(element),
    }
