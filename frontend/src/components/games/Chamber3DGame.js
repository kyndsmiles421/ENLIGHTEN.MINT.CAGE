...
function RockMesh({ hits, hitsPerTarget, color, onStrike, broken }) {
  ...
  // Build a distorted icosahedron once
...
export default function Chamber3DGame({
  ...
  useEffect(() => {
    if (cleared >= targetCount && !done) {
      setDone(true);
      creditSparks(zone, completionXP);
      onComplete && onComplete();
    }
  }, [cleared, targetCount, done, zone, completionXP, onComplete]);
