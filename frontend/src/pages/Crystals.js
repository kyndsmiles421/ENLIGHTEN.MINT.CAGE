  ...
  return (
    <InteractiveModule
      ...
      filters={[
        ...
        ...chakras.slice(0, 6).map(ch => ({
          key: ch, label: ch, count: crystals.filter(c => c.chakra === ch).length,
        })),
      ]}
      filterFn={(item, filter) => item.chakra === filter}
      searchFn={(item, q) => {
        const s = q.toLowerCase();
        return item.name?.toLowerCase().includes(s) || item.aka?.toLowerCase().includes(s) ||
          item.chakra?.toLowerCase().includes(s);
      }}
    />
  );
}
