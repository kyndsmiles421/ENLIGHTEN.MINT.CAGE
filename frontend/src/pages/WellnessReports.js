...
function StatBox({ label, value, color, icon: Icon }) {
  return (
    <div className="rounded-xl p-4 text-center" data-testid={`stat-${label.toLowerCase().replace(/\s/g, '-')}`}
      ...
      <p className="text-[10px] uppercase tracking-wider" style={{ color: 'rgba(248,250,252,0.4)' }}>{label}</p>
    </div>
  );
}
...
function InsightCard({ insight }) {
  return (
    <div className="rounded-xl p-4 flex items-start gap-3"
      ...
      <p className="text-xs" style={{ color: 'rgba(248,250,252,0.7)' }}>{insight.text}</p>
    </div>
  );
}
...
function MoodBar({ mood, count, max, color }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs w-16 text-right capitalize" style={{ color: 'rgba(248,250,252,0.5)' }}>{mood}</span>
      ...
      <span className="text-xs w-6" style={{ color: 'rgba(248,250,252,0.4)' }}>{count}</span>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4 max-w-4xl mx-auto" data-testid="wellness-reports-page">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        {!token ? (
          <p className="text-center text-sm py-12" style={{ color: 'rgba(248,250,252,0.4)' }}>Sign in to view reports</p>
        ) : loading ? (
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={item.label} className="flex items-center gap-2 p-2 rounded-lg" style={{ background: `${item.color}06` }}>
                    ...
                    <span className="text-xs" style={{ color: 'rgba(248,250,252,0.6)' }}>{item.label}</span>
            ...
            {report.insights && report.insights.length > 0 && (
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] mb-3" style={{ color: 'rgba(248,250,252,0.4)' }}>Insights</p>
                <div className="space-y-2">
