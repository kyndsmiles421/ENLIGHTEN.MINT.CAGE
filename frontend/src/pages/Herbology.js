  ...
  return (
    <InteractiveModule
      ...
      filters={[
        ...
        ...allSystems.slice(0, 5).map(s => ({
          key: s, label: s, count: herbs.filter(h => h.systems?.includes(s)).length,
        })),
      ]}
      filterFn={(item, filter) => item.systems?.includes(filter)}
      searchFn={(item, q) => {
        const s = q.toLowerCase();
        return item.name?.toLowerCase().includes(s) || item.latin?.toLowerCase().includes(s) ||
