...
### v1.0.8 — Sleek Hub + Spiritual Shield Hardening (2026-05-01)
- **Sovereign Hub stripped to essentials per user mandate ("user-friendly = clean home page, every module reachable through its pillar"). Removed from front page: Daily Challenges widget, Sovereign Choice panel, Trial Countdown + Signature pills, 4-button action row (Arsenal/Lens/Buy Time/Visitor Shield), Oracle Intent Search bar, Worlds Strip (10 environments), Architect Badge, Seed Hunt widget, Cross-Tradition Pairing widget, the 7 colored "Pull · X" engine pills, and an attempted "Customize Hub" panel. Home page now: title + cross-tradition mark + wallet pills + active mission HUD + AI Time gauge + Compliance Pill + Recall + lattice + utility row + 10 pillars + version stamp. Every removed feature still reachable through its pillar (Challenges → Practice, Oracle → Divination, Settings/Arsenal/Lens/Visitor Shield → Sovereign Council, Worlds → Cosmos & Physics, Seed Hunt → RPG & Adventure, Cross-Tradition → Knowledge).
- **Chamber LEARN button delivers real teachings in 8-20s** (not 60s+ ingress timeouts). `/api/knowledge/deep-dive` now accepts an optional `mode='quick'` flag that routes through gpt-4o-mini with a 22s ceiling and a 350-500 word lesson template, returning real markdown teachings inside the preview ingress 60s window. The chamber's LEARN/TEACH ME FIRST buttons now use this fast path with a 30s axios timeout and a Cancel affordance ("TEACHING… · TAP TO CANCEL") so a slow teaching can be aborted instead of trapping the UI. Live test: 8.8s for "Igneous Rocks · Thin Section Petrography" returning a real 3.4 kB lesson with sensory engagement, mineral identification, and practical micro-actions. Default deep-dive (gpt-5.2, ~1500 words) still available for non-chamber callers. Files: `backend/routes/knowledge.py`, `backend/models.py`, `frontend/src/components/games/ChamberMiniGame.js`.
- **Starseed auth-failure UX hardened.** `startNewAdventure` now does a pre-flight auth check (no Authorization header → toast + redirect to /auth?next=/starseed-adventure). Specific 401 handling on token-mid-flow expiration. Eliminates the "Begin Adventure dead button" symptom that was actually a silent rollback after API 401. (`pages/StarseedAdventure.js`)
- **Avatar fetch 401-spam fixed.** `AvatarBadge` and `AvatarGenerator` now skip `/api/starseed/avatar/:id` entirely when no auth header is present (guest/trial users see the placeholder avatar). (`components/StarseedInventory.js`)
- **Spiritual Shield (defense-in-depth, Play Store framing).** Added `_shield_text` + `_shield_obj` purifier in `routes/starseed_adventure.py` with 30+ regex substitutions (heal→harmonize, healer→harmonizer, healing→harmonizing, treatment→ritual, therapy→ritual practice, medical→mythic, medicine→ritual, diagnose→recognize, prescribe→offer, patient→seeker, doctor→sage, wellness→harmony, remedy→rite, ailment→discord, etc.). Capitalization-preserving, idempotent, word-boundary-safe (does not maul "alignment", "treatise", "uncured"). Applied to (a) the LLM scene generator output, (b) all fallback scenes, (c) `/api/starseed/origins` static origin lore at both registration sites (`routes/starseed.py`, `routes/starseed_adventure.py`). LLM system prompt also explicitly forbids these words at source. Eliminates the "Healing Meadow / cosmic healers" leak the user flagged.
